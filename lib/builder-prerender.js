...
if (!projectRoot) {
    throw new Error("未找到feflow配置文件！");
}

// 最基础的配置
const baseConfig = {
    target: 'node',
    entry: glob.sync(path.join(projectRoot, './src/pages/*')).map((entryItem) => {
        return entryItem.replace("init.js", "prerender-init.js") // 将入口更换为prerender-init.js
    }),
    module: {
        rules: []
    },
...
class PrerenderBuilder extends Builder {
    // static createProdConfig() {
    //     const prodConfig = super.createProdConfig(builderOptions);
    //     // 这里主要是替换entry为prerender-init
    //     // console.log(prodConfig.entry)
    //     const entry = prodConfig.entry
    //     Object.keys(entry).forEach(entryItem => {
    //         let indexEntry = entry[entryItem].replace("init.js", "prerender-init.js")
    //         // console.log(indexEntry)
    //         // const indexPage = require(indexPage);
    //         entry[entryItem] = indexEntry;
    //     })
    //     return prodConfig
    // }
    ...
    static createProdConfig(options) {
        ...
        prodPlugins.push(new CleanWebpackPlugin(['prerender'], {
            root: projectRoot,
            verbose: true,
            dry: false
        }));
    ...
    static setOutput(useHash, pathPrefix, publicPath) {
        ...
        return {
            ...
            path: path.join(projectRoot, `prerender/${pathPrefix}`),
        ...
        return {
            ...
            use: [
                ...
                },
                {
                    loader: 'babel-loader',
                    options: {
                        configFile: path.join(process.cwd(), './.babelrc') // 确保使用的是项目根目录的babel配置
                    }
                }
            ]
        };
    ...
    static setMultiplePage(entries, minifyHtml, inject, inlineCSS, assetsPrefix, htmlPrefix) {
        ...
        Object
            ...
            .map((index) => {
                ...
                // prerender时入口是prerender-init
                const entryFile = `${entry}/prerender-init.js`;
...
const prodConfig = PrerenderBuilder.createProdConfig()
